...
### 方式二：部署到 Cloudflare Workers（推荐）

本项目支持完全部署到 Cloudflare Workers，包括前端和后端。
...
部署成功后，你的应用将通过 Workers URL 提供完整服务：
- 前端：`https://webchat.YOUR-SUBDOMAIN.workers.dev`
- WebSocket API：`wss://webchat.YOUR-SUBDOMAIN.workers.dev/ws`
...
GitHub Repository
       ...
       │                    │
       ↓                    ↓
Raw Content API      Cloudflare Workers
       │                    │
       │                    ├─── 前端文件服务
       │                    ├─── WebSocket API
       │                    └─── Durable Objects
